...
function compose_open_channel_message(contract_address, recipient_address, value, expiration, replica_id)
{
   return ethereumjsabi.soliditySHA3(
        ["address",        "address",        "uint256", "uint256", "uint256"],
        [contract_address, recipient_address, value,    expiration, replica_id]);
...
function sign_open_channel_message(from_account, contract_address, recipient_address, value, expiration, replica_id, callback)
{
    var message = compose_open_channel_message(contract_address, recipient_address, value, expiration, replica_id);
    sign_message(from_account, message, callback);
}
...
async function wait_signed_open_channel_message(from_account, contract_address, recipient_address, value, expiration, replica_id)
{
    let det_wait = true;
    let rez_sign;
    sign_open_channel_message(from_account, contract_address, recipient_address, value, expiration, replica_id, function(err,sgn)
        {   
            det_wait = false;
            rez_sign = sgn
        });
